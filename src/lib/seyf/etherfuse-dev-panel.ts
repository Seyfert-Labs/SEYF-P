...
export function isEtherfuseDevPanelEnabled(): boolean {
  if (process.env.SEYF_ETHERFUSE_DEV_PANEL === 'true' || process.env.SEYF_ETHERFUSE_DEV_PANEL === 'true') return true
  return process.env.NODE_ENV === 'development'
}

export function isEtherfuseSandboxApiHost(): boolean {
  if (process.env.SEYF_ALLOW_SANDBOX_FIAT_SIMULATION === 'true' || process.env.SEYF_ALLOW_SANDBOX_FIAT_SIMULATION === 'true') return true
  try {
