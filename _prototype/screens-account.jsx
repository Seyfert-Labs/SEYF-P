...
/* ---------------- TARJETA ---------------- */
function ScreenCard({ go }) {
  const [frozen, setFrozen] = React.useState(false);
  const [cur, setCur] = React.useState('MXN');
  return (
    <div className="screen screen-enter">
      ...
      <div className="screen-pad">
          ...
          <div className="seg" style={{ marginBottom: 4, border: 'none', background: 'transparent', padding: 0, gap: 8, flexWrap: 'wrap' }}>
            {[
              ...
              <button key={x.c} onClick={() => setCur(x.c)}
                style={{ flex: '1 1 44%', border: '1px solid var(--line)', borderRadius: 14, padding: 14, cursor: 'pointer', textAlign: 'left',
                  background: cur===x.c ? 'var(--accent-soft)' : 'var(--surface-2)' }}>
        ...
        <div className="card" style={{ padding: '4px 18px' }}>
          <div className="list">
            {CARD_TXNS.map(t => <TxnRow key={t.id} t={t} />)}
          </div>
        </div>
      </div>
...
function SecRow({ icon, t, right, last, danger }) {
  return (
...
Object.assign(window, { ScreenCard, ScreenProfile });
