/* UTONOMA — core screens: Onboarding, Home, Wallet */
...
function SecSetupRow({ icon, t, s, on, lock }) {
  ...
  return (
    <div className="card sec-row" style={{ padding: 16 }}>
      ...
      {lock
        ? <span className="pos-pill">Incluido</span>
        : <div className={`tgl ${v ? 'on' : ''}`} onClick={() => setV(!v)} />}
    </div>
  );
}
    ...
    <div className="screen screen-enter">
      ...
      <div className="screen-pad">
        ...
        <div className="card" style={{ padding: '4px 18px' }}>
          <div className="list">
            {TXNS.slice(0, 3).map(t => <TxnRow key={t.id} t={t} go={go} />)}
          </div>
        </div>
      </div>
...
function AcctRow({ go, to, ic, nm, su, vl, series }) {
  return (
    <div className="lrow" onClick={() => go(to)}>
      ...
      <div className="amt" style={{ marginLeft: 8 }}>
        <div className="a num">${FMT(vl, 0)}</div>
      </div>
      ...
      <div className="screen-pad">
        <div className="card glow" style={{ padding: 24, textAlign: 'center' }}>
          ...
          <p className="amount num" style={{ fontSize: 46, marginTop: 12 }}>
            $48,250<span style={{ opacity: .5 }}>.40</span>
          </p>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginTop: 14 }}>
            <span className="pos-pill"><Icon name="leaf" size={12}/> +$12.40 hoy</span>
            <span style={{ fontSize: 13, color: 'var(--txt-muted)' }}>9% anual · pagado diario</span>
          </div>
        </div>

        <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
          <button className="btn btn-primary" style={{ flex: 1 }}><Icon name="plus" size={18}/> Agregar</button>
          <button className="btn btn-ghost" style={{ flex: 1 }} onClick={() => go('convertir')}><Icon name="send" size={18}/> Enviar</button>
        </div>
        ...
        <div className="sec-head"><h3>Movimientos</h3><span className="link">Filtrar</span></div>
        <div className="card" style={{ padding: '4px 18px' }}>
          <div className="list">
...
Object.assign(window, { Onboarding, ScreenHome, ScreenWallet });
